...
void game::start()
{
	...
	Uint32 start;
        ...
        while(running)
        {
			...
			if(playerCam_->cam.getPitch_())
			  playerCam_->cam.setPitch(1.5f);
			
			if(playerCam_->cam.getPitch2_())
			  playerCam_->cam.setPitch2(1.5f);
			  
			 if(playerCam_->cam.getYaw_()) 
			 playerCam_->cam.setYaw(1.5f);
			 
			if(playerCam_->cam.getYaw2_()) 
			 playerCam_->cam.setYaw2(1.5f);
